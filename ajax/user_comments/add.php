...
$check = $conn->prepare("SELECT * FROM comments WHERE content_id = ? AND user_id = ? AND comment = ?");
$check->execute([$content_id, $user_id, $comment]);

if ($check->rowCount() > 0) {
    echo json_encode([
        'status' => 'error',
        'message' => 'Такой комментарий уже существует'
    ]);
    exit;
}
...
$ins = $conn->prepare("
    INSERT INTO comments (id, content_id, user_id, tutor_id, comment)
    VALUES (?, ?, ?, ?, ?)
");
