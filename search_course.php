...
<body>
    ...
    <section class="courses">
        ...
        <!-- Исправьте в форме на этой же странице -->
        <form action="" method="post" class="search_tutor">
            <input type="text" name="search_course" maxlength="100" required placeholder="Поиск курса по названию"
                value="<?= isset($_POST['search_course']) ? htmlspecialchars($_POST['search_course']) : '' ?>">
            ...
            if (isset($_POST['search_course_btn']) && !empty($_POST['search_course'])) {
                ...
                if ($select_courses->rowCount() > 0) {
                    while ($fetch_courses = $select_courses->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        <div class="box">
                            <div class="tutor">
                                <img src="uploaded_files/<?= htmlspecialchars($fetch_tutor['image'] ?? 'default.png') ?>"
                                    alt="Tutor">
                                <div>
                            ...
                            <img src="uploaded_files/<?= htmlspecialchars($fetch_courses['thumb']) ?>" class="thumb"
                                alt="Course thumb">
